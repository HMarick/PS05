...
In exploring this dataset, we first examined the relationships between the quantitative predictor variables and our response variable. Therefore, we created scatterplots between count and the following predictors: temp, atemp, humidity, and windspeed. 
...
```{r}
train$datetime = structure(train$datetime,class=c('POSIXt','POSIXct'))
...
test$pred<-exp(predict.lm(mod, test))
submission<-test[,c(1,11)]
