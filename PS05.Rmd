...
Before examining any relationships between variables, we looked at the distribution of the number of bike rentals.

```{r}
ggplot(train, aes(count)) + geom_histogram() + ggtitle("Distribution of Bike Rentals") + xlab("Bike Rentals")
```
Notice the severe right skew on this dataset. This indicates that it might be useful to apply a log() transformation to our response variable when creating our model.

We then examined the relationships between the quantitative predictor variables and our response variable. Therefore, we created scatterplots between count and the following predictors: temp, atemp, humidity, and windspeed. 
...
Ultimately, we elected to fit a linear model with temperature, humidity, and time of day. In order to ensure that the distribution of bike rentals was not significantly right skewed, we took the log() of the total bike rentals. In addition, this transformation ensured that our predictions are always positive. The $R^2$ value for this model was 0.7692, which indicates that our model learned the training set well. Naturally, the concern with an $R^2$ value this high is that we may be overfitting a specific training set. Having said that, with only 3 variables that should impact bike rentals universally, overfitting should not be an issue here.
